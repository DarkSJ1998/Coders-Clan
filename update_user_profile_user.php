...
<?php
	...
	$check = "SELECT * FROM `user` WHERE `user_id` = '$id' AND `user_password` = '$pwd1'";
	$result = $connection->query($check);
	if($result->num_rows == 1)
	{
		$query = "UPDATE `user` SET `name` = '$uname', `user_password` = '$pwd3', `email_id` = '$email', `ph_no` = '$ph_no', `section` = '$section', `dept` = '$dept' WHERE `user_id` = '$id'";
		if($connection->query($query) === TRUE)
		{
			echo "Update successful";
			echo "<br><a href='user_profile_user.php'> Go to Profile </a>";
			$_SESSION['name'] = $uname;
