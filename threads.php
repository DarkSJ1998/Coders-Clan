...
<marquee style="background:#b70e0e;font-family:Supercell;color:white;" behavior="alternate"> Threads </marquee>
<center>
<?php
	include ('connection.php');
	
	$query = "SELECT * FROM `thread` WHERE `type` = 'Question' ORDER BY `timestamp` DESC";
	...
	else
	{
		...
		while ($row=mysqli_fetch_row($result))		//substr($row[5],0,140) .
		{
			...
			<td>
			<div>
			<a href='show_thread.php?thread=$row[0]'><h5>[Q] $row[1]</h5></a>
